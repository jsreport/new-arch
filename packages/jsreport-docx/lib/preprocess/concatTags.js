...
module.exports = (files) => {
  for (const f of files.filter(f => f.path.endsWith('.xml'))) {
    ...
    const drawningTextElements = doc.getElementsByTagName('a:t')
    concatTextNodes(generalTextElements)
    concatTextNodes(drawningTextElements)
  }
}

function concatTextNodes (elements) {
  const toRemove = []
