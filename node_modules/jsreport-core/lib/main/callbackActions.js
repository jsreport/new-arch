...
async function resolveEntityFromPath (reporter, originalReq, spec) {
  const localReq = reporter.Request(originalReq)
  return reporter.folders.resolveEntityFromPath(spec.path, spec.entitySet, localReq)
}

module.exports = (reporter) => (data, req) => {
  switch (data.action) {
    case 'render': return render(reporter, req, data.data)
    ...
    case 'folders.resolveEntityFromPath': return resolveEntityFromPath(reporter, req, data.data)
  }
}
