...
class MainReporter extends Reporter {
  ...
  async init () {
    ...
    try {
      ...
      if (!this.options.templatingEngines.safeSandboxPath) {
        this.options.templatingEngines.safeSandboxPath = path.join(__dirname, 'worker/render/safeSandbox.js')
      }
      ...
      const extensionsInWorkers = this.extensionsManager.extensions.filter(e => e.worker)

      this.scriptManager = this.options.templatingEngines.scriptManager || new ScriptsManager({
        ...this.options,
        extensionsDefs: extensionsInWorkers.map(e => JSON.parse(JSON.stringify(e))),
        requestContextMetaConfigCollectionStr: JSON.stringify(requestContextMetaConfigCollection)
      ...
      this.logger.debug(`Extensions in workers: ${extensionsInWorkers.map((e) => e.name).join(', ')}`)
  ...
  async render (req, parentReq) {
    ...
    req.context.requestId = nanoid()

    this._requestsMap.set(req.context.requestId, req)
    ...
    try {
      responseResult = await this.scriptManager.execute({
        ...
        execModulePath: path.join(__dirname, '/worker/render/startScript.js'),
        useReporter: true,
        onLog: (log) => {
          this.logger[log.level](log.message, { ...req, timestamp: log.timestamp })
        },
        callback: (data, cb) => {
          const req = this._requestsMap.get(data.requestId)
          this.callbacks(data, req).then((r) => cb(null, r)).catch(e => cb(e))
        }
      })

      Object.assign(response, responseResult)

      await this.afterRenderListeners.fire(req, response)

      response.stream = streamifier.createReadStream(response.content)
    } catch (err) {
      await this.renderErrorListeners.fire(req, response, err)

      throw err
    } finally {
      this._requestsMap.delete(req.context.requestId)
    }
