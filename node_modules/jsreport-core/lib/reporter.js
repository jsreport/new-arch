 ...
 */
  async render (req, parentReq) {
    ...
    let res
    try {
      res = await this.scriptManager.executeAsync({
        req,
        parentReq,
        action: 'render'
      }, {        
        timeout: this.options.reportTimeout,
        execModulePath: path.join(__dirname, '/worker/worker.js'),     
        callback: (data, cb) => {       
  ...
  async _initWorker () {    
    ...
    for (let i = 0; i < this.options.templatingEngines.numberOfWorkers; i++) {
      promises.push(this.scriptManager.executeAsync({
        ...this.options,
        extensionsDefs: this.extensionsManager.extensions.filter(e => e.worker).map(e => JSON.parse(JSON.stringify(e))),    
    ...
    this.logger.info(`${this.options.templatingEngines.numberOfWorkers} worker threads initialized in ${new Date().getTime() - start}ms`)
  }
