...
module.exports = (_sandbox, options = {}) => {
  ...
  return {
    ...
    decontextifyValue: (value) => decontextifyVal(contextified, proxied, value),
    restore: () => {
...
function decontextifyVal (contextified, proxied, value) {
  let newValue

  if (proxied.has(value)) {
    newValue = proxied.get(value)
  } else {
    newValue = contextified.has(value) ? contextified.get(value) : value
...
function restoreProperties (context, originalValues, contextified, proxied) {
  ...
  Object.keys(originalValues).sort(sortPropertiesByLevel).forEach((prop) => {
    ...
    for (let i = 0; i <= lastPartsIndex; i++) {
      ...
      if (currentContext) {
        ...
        // unwrapping proxies
        value = decontextifyVal(contextified, proxied, value)
  ...
  // decontextify top level properties
  Object.keys(newContext).forEach((prop) => {
    newContext[prop] = decontextifyVal(contextified, proxied, newContext[prop])
  })
