...
const engineScript = promisify(require('./engineScript'))
let reportCounter = 0
...
async function invokeRender (reporter, request, response) {
  ...
  reporter.logger.debug(`Rendering engine ${request.template.engine.name}`, request)

  const engineRes = await engineScript({
