...
class Document extends Readable {
  ...
  async end() {
    ...
    try {
      ...
      // also we create from props extra objects that may be encrypted and protected with password
      ...
      for (const key in this.info) {
        if (this.info[key] != null) { 
          const propObject = new PDF.Object()
          await this._registerObject(propObject)

          let value = new PDF.String(this.info[key])
          if (this.security) {
            value = this.security.getEncryptFn(propObject.id)(Buffer.from(this.info[key])).toString('binary')
            value = value.replace(escapableRe, c => escapable[c]);
            value = '(' + value + ')'
          }         
          propObject.content = value
          await this._writeObject(propObject)
          infoObject.properties.set(key, propObject.toReference())
        }
      }
